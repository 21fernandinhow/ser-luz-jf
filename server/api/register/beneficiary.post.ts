import { validateRegisterBeneficiary } from '~/server/utils/validation'
...
export default defineEventHandler(async (event) => {
  ...
  const validation = validateRegisterBeneficiary(body)
  if (!validation.ok) {
  ...
  if (authError) {
    ...
    console.error('[register/beneficiary] Auth error:', authError.message)
    throw createAppError({
      code: 'INTERNAL',
  ...
  if (insertError) {
    console.error('[register/beneficiary] Profile insert error, reverting Auth user:', insertError.message)
